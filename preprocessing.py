...
def population_split(df, role_child='role_child', age='age', union=True):
    ...
    if union:
        df['_minor'] = numpy.where((df['_minor1'] == 1) | (df['_minor2'] == 1), 1, 0)
    else:
        df['_minor'] = numpy.where((df['_minor1'] == 1) & (df['_minor2'] == 1), 1, 0)
    
    df.drop(columns=['_minor1', '_minor2'], inplace=True)
    
    minors = df[df['_minor'] == 1].drop(columns=['_minor'])
    majors = df[df['_minor'] == 0].drop(columns=['_minor'])
...
def sarahs_rule(majors, minors, REL):
    ...
    """
    fields = ['referralid', 'referralpersonid', 'age', 'firstname', 'lastname', 'female', 'role_parent']
    newrel = minors[fields].merge(majors[fields], how="outer", on='referralid')
    newrel = newrel[newrel.referralpersonid_x.notna()]

    newrel = newrel[(newrel['role_parent_y'] == 1) & (newrel['female_y'] == 1)]
    rule1 = newrel.drop_duplicates(['referralid', 'referralpersonid_x'], keep=False)
    rule1 = rule1[['referralid', 'referralpersonid_x', 'referralpersonid_y']]
    rule1 = rule1.rename(columns={'referralpersonid_x': 'allegedvictimid', 'referralpersonid_y': 'referralpersonid'})
    rule1['relationship'] = 'Mother-Biological'

    newrel = newrel[newrel.duplicated(['referralid', 'referralpersonid_x'], keep=False)]
    newrel = newrel[newrel['age_y'] - newrel['age_x'] >= 14]
    rule2 = newrel.drop_duplicates(['referralid', 'referralpersonid_x'], keep=False)
    rule2 = rule2[['referralid', 'referralpersonid_x', 'referralpersonid_y']]
    rule2 = rule2.rename(columns={'referralpersonid_x': 'allegedvictimid', 'referralpersonid_y': 'referralpersonid'})
    rule2['relationship'] = 'Mother-Biological'

    newrel = newrel[newrel['lastname_x'] == newrel['lastname_y']]
    rule3 = newrel.drop_duplicates(['referralid', 'referralpersonid_x'], keep=False)
    rule3 = rule3[['referralid', 'referralpersonid_x', 'referralpersonid_y']]
    rule3 = rule3.rename(columns={'referralpersonid_x': 'allegedvictimid', 'referralpersonid_y': 'referralpersonid'})
    ...
    REL.drop_duplicates(['referralid', 'allegedvictimid', 'referralpersonid'], inplace=True, keep='first')
...
def children(majors, minors, REL):
    ...
    # Choose best moms
    REL['prior'] = 0
    REL['prior'] = numpy.where(REL['relationship'].str.contains('Mother'), 3, REL['prior'])
    REL['prior'] = numpy.where(REL['relationship'] == 'Mother-Adoptive', 2, REL['prior'])
    REL['prior'] = numpy.where(REL['relationship'] == 'Mother-Biological', 1, REL['prior'])
    moms = REL[REL['prior'] != 0]
    moms.sort_values(by=['referralid', 'allegedvictimid', 'prior'], inplace=True)
    moms.drop_duplicates(subset=['referralid', 'allegedvictimid'], keep='first', inplace=True)
    
    # Choose best dads
    REL['prior'] = 0
    REL['prior'] = numpy.where(REL['relationship'].str.contains('Father'), 3, REL['prior'])
    REL['prior'] = numpy.where((REL['relationship'] == 'Father-Adoptive') | 
                               (REL['relationship'] == 'Father-Legal'), 2, REL['prior'])
    REL['prior'] = numpy.where(REL['relationship'] == 'Father-Biological', 1, REL['prior'])
    dads = REL[REL['prior'] != 0]
    dads.sort_values(by=['referralid', 'allegedvictimid', 'prior'], inplace=True)
    dads.drop_duplicates(subset=['referralid', 'allegedvictimid'], keep='first', inplace=True)
    ...
    # Merge dad's info
    family = minors.merge(moms_rel, how='left', left_on=['referralid', 'referralpersonid'],
                          right_on=['referralid', 'allegedvictimid'])
    family.drop(columns=['allegedvictimid', 'referralpersonid_y', 'firstname_y', 'lastname_y'], inplace=True)
    family.rename(columns={'referralpersonid_x': 'referralpersonid', 'firstname_x': 'firstname',
                           'lastname_x': 'lastname', 'SDX': 'SDX_mom', 'dob_x': 'dob', 'ssn_x': 'ssn',
                           'mciid_x': 'mciid', 'dob_y': 'dob_mom', 'ssn_y': 'ssn_mom', 'mciid_y': 'mci_mom'}, inplace=True)

    # Merge mom's info        
    family = family.merge(dads_rel, how='left', left_on=['referralid', 'referralpersonid'],
                          right_on=['referralid', 'allegedvictimid'])
    family.drop(columns=['allegedvictimid', 'referralpersonid_y', 'nroles', 'firstname_y', 'lastname_y'], inplace=True)
    family.rename(columns={'referralpersonid_x': 'referralpersonid', 'firstname_x': 'firstname',
                           'lastname_x': 'lastname', 'SDX': 'SDX_dad', 'dob_x': 'dob', 'ssn_x': 'ssn',
                           'mciid_x': 'mciid', 'dob_y': 'dob_dad', 'ssn_y': 'ssn_dad', 'mciid_y': 'mci_dad'}, inplace=True)
