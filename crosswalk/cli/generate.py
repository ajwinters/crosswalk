...
writes match_dfA.csv and match_dfB.csv once, and `crosswalk-match` loads them
going forward. Unlike regenerating from the seed each run, the files can be
opened, inspected, and joined back to matcher output for analysis.
...
Run as `crosswalk-generate --n-records 10000` (after `pip install -e .`) or
`python -m crosswalk.cli.generate --n-records 10000`.
"""
...
DATA_DIR = "data"   # default output dir, relative to the current working directory
